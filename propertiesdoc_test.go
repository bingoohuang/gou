...
func Test_Load(t *testing.T) {
	...
	p, err := LoadProperties(strings.NewReader(s))
	if nil != err {
		t.Error("加载失败")
		return
	}
...
# 注释
	...
	doc, _ := LoadProperties(bytes.NewBufferString(str))
	buf := bytes.NewBufferString("")
	SavePrettyProperties(doc, buf)
...
func Test_Set(t *testing.T) {
	...
	doc.Set("NOT-EXIST", "Setup-NewProperties-Item")
	newValue, _ = doc.Get("NOT-EXIST")
	expect(t, "修改不存在的项,默认是新增行为", "Setup-NewProperties-Item" == newValue)
}
