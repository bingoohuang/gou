...
//
// ChunkTimeout: Read() returns if no data has been read for the specified time,
//                     even if the overall timeout has not been hit yet.
//
...
//     nbr := nbreader.New(reader, 1 << 16,
//                   nbreader.Timeout(2 * time.Second), nbreader.ChunkTimeout(200 * time.Millisecond))
package nbreader
...
// ChunkTimeout allows to set the timeout after which the end of
// a chunk of data is assumed and the read data is returned by Read().
func ChunkTimeout(duration time.Duration) Option {
	return func(r *NBReader) { r.chunkTimeout = duration }
}

// Timeout allows to set the timeout after which Read() returns, even if no data has been read.
func Timeout(duration time.Duration) Option {
	return func(r *NBReader) { r.forceTimeout = duration }
}

// New returns a new NBReader with the given block size.
func New(reader io.Reader, blockSize int, options ...Option) *NBReader {
	r := &NBReader{reader: reader, dataChan: make(chan []byte), blockSize: blockSize}
...
func (r *NBReader) Read(buffer []byte) (int, error) {
	...
	for r.buffer.Len() < len(buffer) {
		lastStart := time.Now()
		remaining = r.forceTimeout - time.Since(start)
		...
		_, err := r.readWithTimeout(nextTimeout)
		...
		if time.Since(start) >= r.forceTimeout {
			break
		}
	}
...
func (r *NBReader) readWithTimeout(timeout time.Duration) (int, error) {
	select {
